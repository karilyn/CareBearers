...
//Popup for when a user clicks on a request's Details button
const RequestPopup = (props) => {
  //Get kids of parent who made request
  ...
  //Get parent's ratings - need to revisit this to do in the backend after demo day
  for (const item of props.completedReservations) {
    if (item.parent_id === props.popupData.id) {
      for (const review of props.reviews) {
        if (
          review.reservation_id === item.id &&
          review.reviewer_id !== props.popupData.id
        ) {
      ...
      <div className="popup-request__inner">
        ...
        <div className="popup-request__inner__parent">
          ...
          <div className="popup-kids">
            <h5 className="request-popup">
              <strong>{props.popupData.first_name}'s Kids: </strong>
            </h5>
            {myKids.map((kid) => {
              return (
                <div className="popup-request-card__img">
                  <div
                    className="card-img-top__background request"
                    key={kid?.id}
                  >
                    <img
                      src={kid.photo_url}
