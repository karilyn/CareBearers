...
//Renders pending care requests for logged in caregivers
const Requests = () => {
  ...
  const clickButton = (status, id) => {
    fetch(`${state.url}/reservations/${id}`, {
      ...
      .then((data) => {
        ...
        //update state once request is accepted or declined
              ...
              return (
                <div className="request-card" key={res?.id}>
                  <div className="card-body">
                    ...
                    <p className="card-text">
                      <strong>Address:</strong> {res?.street} {res?.city},{" "}
                      {res?.province}
                    </p>
                    <p className="card-text">
                      <strong>Reservation for:</strong>{" "}
                      {getParentDetails(parents, res?.parent_id)?.first_name}{" "}
                      {getParentDetails(parents, res?.parent_id)?.last_name}
                    </p>
                    <p className="card-text">
                    ...
                    <div className="button-container">
                      <button
                        ...
                        onClick={() => clickButton(1, res?.id)}
                      >
                        Accept
                      </button>
                      <button
                        className="btn request decline"
                        onClick={() => clickButton(2, res?.id)}
                      >
                        Decline
                      </button>
                      <button
                        className="btn request details"
                        onClick={() => onClickDetails(res)}
                      >
                        Details
                      </button>
                    </div>
