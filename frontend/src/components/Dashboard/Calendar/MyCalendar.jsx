...
//Calendar of reservations for logged in user using react-big-calendar
const MyCalendar = (props) => {
  ...
  const myEventsList = events.map((event) => {
    return {
      ...
      end: moment(event.start_time)
        .add(event.duration_in_minutes, "m")
        .toDate(),
      title:
      ...
      <div className="event">
        {clicked ? (
          <Event
            ...
            start={moment(clickedEvent.start).format("MMM Do YYYY, h:mm a")}
            end={moment(clickedEvent.end).format("h:mm a")}
